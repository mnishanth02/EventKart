...
const telemetrySdkTimeoutMs = 30_000;
...
describe("otel", () => {
	...
	it(
		"initTelemetry returns a handle with SDK when no Sentry DSN",
		async () => {
		...
		telemetrySdkTimeoutMs,
	);

	it(
		"initTelemetry configures metrics reader when OTLP endpoint is set",
		async () => {
			const { initTelemetry } = await import("../../src/lib/otel.js");
			const handle = initTelemetry({
				OTEL_SERVICE_NAME: "test-service",
	...
	it(
		"initTelemetry parses OTEL headers correctly",
		async () => {
			const { initTelemetry } = await import("../../src/lib/otel.js");
			const handle = initTelemetry({
				OTEL_SERVICE_NAME: "test-service",
	...
	it(
		"initTelemetry uses default service name when not provided",
		async () => {
			const { initTelemetry } = await import("../../src/lib/otel.js");
			const handle = initTelemetry({});
			expect(handle.sdk).not.toBeNull();
	...
	it(
		"initTelemetry creates standalone MeterProvider when Sentry + OTLP endpoint",
		async () => {
			const { initTelemetry } = await import("../../src/lib/otel.js");
			const handle = initTelemetry({
				SENTRY_DSN: "https://examplePublicKey@o0.ingest.sentry.io/0",
	...
	it(
		"initTelemetry respects custom metrics export interval",
		async () => {
			const { initTelemetry } = await import("../../src/lib/otel.js");
			const handle = initTelemetry({
				OTEL_EXPORTER_OTLP_ENDPOINT: "http://localhost:4318",
	...
	it(
		"shutdownTelemetry handles SDK handle",
		async () => {
	...
	it(
		"shutdownTelemetry handles MeterProvider handle",
		async () => {
			const { initTelemetry, shutdownTelemetry } = await import(
				"../../src/lib/otel.js"
			);
