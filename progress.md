...
| 50  | Phase 2 Readiness — W1.1: `organizers.slug` column + deterministic backfill + reserved-slug list + organizer slug service + `slug_redirects` writes (migration 0015) | — | ✅ Done | 2026-04-29 | 2026-04-29 |
...
| 52  | Phase 2 Readiness — W1.3: Tiered post-publish edits (I-1.2.8): `publishedEventPatchSchema`, `EVENT_UPDATE_PUBLISHED` audit constant + constrained metadata, 409 with `requiresUnpublish: true` for high-risk fields, audit-classified every event update function, unlocked `updateEventPolicies` for published events | — | ✅ Done | 2026-04-29 | 2026-04-29 |
...
| 54  | Phase 2 Readiness — W1.5: `events.first_published_at` column (migration 0017), set once in `publishEvent` + `adminApproveEvent`, never cleared on unpublish; rewrote `getPublishedPaidEventCount` to use it; closes unpublish→republish bypass of first-3-paid admin gate (security fix) | — | ✅ Done | 2026-04-29 | 2026-04-29 |
...
| Phase 1 | 1.2 — Event Creation & Management                   | ✅ Complete    | All 10 items (I-1.2.1 through I-1.2.10) complete. Phase 2 Readiness sprint (2026-04-29) further hardened: capacity columns `spots_total`/`spots_remaining` shipped (migration 0016, W1.2); tiered post-publish edits fully implemented with `publishedEventPatchSchema` + `EVENT_UPDATE_PUBLISHED` audit + 409 `requiresUnpublish` (W1.3); `first_published_at` closes the unpublish→republish bypass of the first-3-paid gate (migration 0017, W1.5). api 725/725, web 163/163 passing. |
| Phase 2 Readiness | — (cross-cutting Phase 1/2 hardening)  | ✅ Complete    | Sprint completed 2026-04-29 (commits c04b9f1→824549e). W1.1: organizer slug + backfill + reserved slugs (migration 0015). W1.2: `event_categories` capacity columns + CHECK constraints (migration 0016). W1.3: tiered post-publish edits — `publishedEventPatchSchema`, `EVENT_UPDATE_PUBLISHED` audit, 409 `requiresUnpublish`. W1.4: `buildTestAppWithoutKey()` + `loadConfig` regression test (closes I-0.2.15). W1.5: `events.first_published_at` + first-3-paid security fix (migration 0017). W2.1: log-only email stubs at I-1.1.9–12 / I-1.2.11–12 + `EMAIL_JOB_NAMES` enum + `emitEmailStub()` helper. W2.3: presigned upload Content-Length hardening (`createPresignedPost`). Validation: api 725/725, web 163/163, shared 168/168, db 31/31. **Phase-3 deferrals:** real Resend templates/processors (I-3.3.1), parental consent enum (I-3.1.7), conversion-funnel emission (I-0.4.7), atomic capacity decrement (I-3.2.10), secret-rotation runbook (I-0.2.13), pre-launch gates. |
