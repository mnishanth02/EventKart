---
title: Kiran V1 — Structured Requirements Document
version: 1.1
...
# Kiran V1 — Requirements Document
...
**Who:** Anyone browsing Kiran without logging in.
...
- Request support/dispute resolution through Kiran

### 1.3 Organizer (Authenticated, Verified)

**Who:** A fitness event organizer who signs up, gets verified, and uses Kiran to create and manage events.
...
- Request refund processing through Kiran ops

### 1.4 Kiran Ops / Admin (Internal)

**Who:** Kiran's internal operations team managing the platform during the pilot.
...
| **Admin panel** | Kiran ops team | Full login |
...
- `/admin/*` — Kiran ops panel (verifications, reviews, disputes)
...
- **F-3.2.3** — Split payout configuration (Kiran fee captured at payment time, remainder settled to the organizer-linked account)
...
- **F-4.1.4** — Basic revenue view per event (total collected, Kiran fee, net to organizer)
...
- **F-5.3.2** — Roster includes: name, category, payment status, and bib only if assigned outside Kiran
...
- **F-7.1.4** — Handling for already-settled funds (organizer responsibility, Kiran mediation)
...
- Verification must be explained as a Kiran onboarding and policy check, not a blanket guarantee of event quality or safety
- Participants must be able to report issues through Kiran support, not only through the organizer
