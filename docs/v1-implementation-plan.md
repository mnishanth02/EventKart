...
| Production deployment infrastructure          | 🟡 In progress | Code + Docker + Railway config-as-code + CI/CD complete (Phases 1–3, 5, 7 of `docs/impl-plan/infrastructure-railway-deployment.md`); Railway provisioning + DNS + live verification (Phases 4, 6) deferred to launch-prep window. Runbook: `docs/operations/railway-deployment-setup.md`. |
...
| I-1.2.1: Event creation form | ✅ Complete | Events table expanded with V1 event creation fields, shared Zod schemas/constants, `POST /api/v1/events` with organizer auth + CSRF, and `/org/events/new` TanStack Form UI for draft single-day paid running events in Coimbatore. **Bugfix 2026-05-02:** `/org/events/new` now wraps in `<OrganizerSetupGate>` (mirrors dashboard) and `createDraftEvent` enforces `hasAcceptedAllPolicies` (403 if missing); Coimbatore-only city/state DB CHECK constraints relaxed (migration 0019) to bounded free-text defaults — country (India), timezone (Asia/Kolkata), and paid-only locks retained. See progress.md row 91. |
...
**v2.x known follow-up — flagged during Module 2.4 (2026-04-30):**

- **Set-Cookie + public-cache leak risk (auth.ts global onRequest hook) — ✅ Fixed (2026-05-01)** — `apps/api/src/plugins/auth.ts` now strips non-deletion `Set-Cookie` headers from responses that carry `Cache-Control: public`, while preserving stale-session deletion cookies. Regression coverage in `apps/api/test/plugins/auth.test.ts` pins both sides of the contract: public cacheable responses cannot emit session-creation cookies, and stale-cookie deletions still reach the browser. Caught by adversarial review during I-2.4.6.
...
| 2 ✅  | I-1.2.1  | Event creation form — V1-allowed events only (single-day, paid running, Indian city)     | ✦       | ✦        | ✦      | I-1.2.10, **I-1.1.1**, **I-0.3.3**                | Complete: events table fields + migration, shared validation/constants, `POST /api/v1/events` with organizer auth + CSRF and slug uniqueness, `/org/events/new` draft form UI. **Bugfix 2026-05-02:** Coimbatore-only city/state DB CHECK relaxed (migration 0019) to bounded free-text defaults; `/org/events/new` gated by `<OrganizerSetupGate>` + `createDraftEvent` enforces `hasAcceptedAllPolicies`. Country/timezone/paid locks retained. See progress.md row 91.                                                                                                                                                                                                                                       |
...
| 5 ✅  | I-2.1.5  | Share-optimized previews — Open Graph meta tags                                                      | —       | ✦        | —      | I-2.1.1                                            | **Complete (2026-04-29)** — `buildPublicEventMeta` helper emits the OG core (title/description/type/site_name/locale) + Twitter card text fields + canonical/`og:url` (when `VITE_SITE_URL` is set), with grapheme-aware truncation via `Intl.Segmenter` and pre-truncate normalization that strips C0/C1/DEL/bidi controls while preserving ZWJ. **Text-only by design** — `og:image` deferred until a stable image-serving route exists (presigned hero URLs expire in 1h vs days/weeks of OG cache). Two GPT-5.5 reviews (all findings adopted).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| 6 ✅  | I-2.1.6  | Structured data markup — JSON-LD for search discovery                                                | —       | ✦        | —      | I-2.1.1                                            | **Complete (2026-04-29)** — `buildPublicEventJsonLd` helper emits Schema.org `Event` JSON-LD on `/events/:slug` via `head().scripts`; `serializeJsonLdForInlineScript` mirrors TanStack Router's framework `escapeHtml` (`& > < \u2028 \u2029`) so hostile `</script>` payloads in description/venue/organizer fields cannot escape the inline `<script type="application/ld+json">`. `Offer.price` is a JSON Number (`basePrice / 100`); `isAccessibleForFree` always emitted; `image` deferred (parity with I-2.1.5 D5). Pure deterministic helper preserves the I-2.1.1 CDN cache contract. 28 unit tests; two GPT-5.5 reviews (all findings adopted).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| 7 ✅  | I-2.1.7  | "Register Now" CTA linking to booking flow                                                           | —       | ✦        | —      | I-2.1.1                                            | **Complete (2026-04-29)** — state-aware CTA on the sidebar Card + mobile bottom bar; SSR baseline `"unknown"` keeps cached HTML truthful with neutral copy + an active link to the Phase-3 placeholder. Post-mount `useRegistrationState` derives `open` / `not_yet_open` / `closed_window` / `event_ended` (precedence `event_ended > closed_window > not_yet_open > open`) from the three time bounds, with defensive parsing (`endAt` fail-safe → `event_ended`; `closesAt` fail-closed → `closed_window`). Boundary refresh via a single chained `setTimeout` capped at `2^31 - 1` ms (no polling, no `TimeoutOverflowWarning`). Disabled CTA uses native `<Button disabled aria-disabled="true" aria-describedby>` so it drops from tab order. Phase-3 placeholder route at `/events/:slug/register` is `noindex,nofollow` and reuses the I-2.1.1 CDN cache contract. Sibling-route refactor: `$slug.tsx` → `$slug/index.tsx`. 24 new tests; GPT-5.5 plan + code review (4 plan-review findings adopted, code review no significant issues).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| 8 ✅  | I-2.1.9  | Spots-remaining badge — real-data only                                                               | ✦       | ✦        | —      | I-2.1.1, **I-1.2.2**, **I-3.2.10**                 | **Complete (2026-04-30)** — `PublicEventSpotsRemainingBadge` rendered inside `PublicEventPricingBreakdown` with two variants: `<Badge variant="destructive">Sold out</Badge>` + `Ban` icon for `spotsRemaining === 0`; `<Badge variant="outline">{n} spot{s?} remaining</Badge>` + amber tint + `Users` icon for `0 < remaining ≤ min(25, ceil(0.20 × spotsTotal))` (hybrid absolute cap + 20% ratio). Public schema extended with wrapped+nullable `capacity: { spotsTotal, spotsRemaining } \| null` (`superRefine(remaining ≤ total)`) — forward-compat with future "unlimited" toggle. API service projects `capacity: null` with Pino warn on invariant violation (fail-soft so corrupted rows never 5xx the page). Display gate hides on SSR baseline `"unknown"` AND `"closed_window"` / `"event_ended"` so cached HTML never advertises stale availability past a window-close boundary. Desktop column header gated on `hasAnySpotsBadge` (mirrors I-2.1.4 `hasEarlyBird`); mobile per-row inline `<dt>/<dd>`. `data-surface="desktop" \| "mobile"` disambiguates the dual-rendered DOM. **All web rendering gated by `VITE_PUBLIC_SPOTS_REMAINING_BADGE_ENABLED` (default `false`) — implementation ships dark, production turns on with a one-line env flip once I-3.2.10 atomic decrement lands.** Flag uses custom `stringBoolEnv` schema (NOT `z.coerce.boolean()`, which parses string `"false"` as `true` due to JS truthiness — caught as the sole Critical in post-impl GPT-5.5 code review and fixed). Three workspaces touched (`packages/shared`, `apps/api`, `apps/web`); no DB changes; CDN cache contract preserved. **387 web tests (+54), 769 API tests (+1), 169 shared tests** — including 31 helper tests covering threshold edges (totals 1, 2, 5, 50, 100, 124, 125, 126, 1000, `MAX_SAFE_INTEGER`), 13-test page suite, 10-test env-parser regression, and the API fail-soft regression test. GPT-5.5 plan review (2 Critical + 5 Important + 4 Improvement findings; all Critical + most Important adopted) + GPT-5.5 code review (1 Critical adopted, fix-loop applied, re-review APPROVED). |
| 9 ✅  | I-2.1.10 | Early-bird countdown timer — real-data only                                                          | —       | ✦        | —      | I-2.1.1, **I-1.2.3**                               | **Complete (2026-04-30)** — `PublicEventEarlyBirdCountdown` badge on the sidebar Card + mobile sticky bar (single source of truth across both surfaces). `getNextActiveEarlyBirdCutoff` picks the soonest in-future **effective cutoff** = `min(earlyBirdDeadline, registrationClosesAt, endAt)` per tier and rejects tiers whose `registrationOpensAt >= effectiveCutoff` (offer unreachable). Reuses `hasValidEarlyBirdOffer` legacy guard so the badge and the I-2.1.4 breakdown column never disagree. Auto-rolls to the next eligible tier when the soonest cutoff passes; auto-hides when no tier qualifies; gated off in `closed_window` / `event_ended`. Format: `Xd Yh` ≥ 24h / `Xh Ym` ≥ 1h / `Xm` ≥ 1m / `<1m`. Tick scheduler `delay = msUntilCutoff − floor(remainingMs/60_000)*60_000 + 1` lands precisely on each minute-floor boundary with **no global per-minute `+1ms` drift** (Critical-2 from plan review). SSR-safe `useEarlyBirdCountdown` returns `null` until mount. No `aria-live` on the dual-surface badge to avoid double-announce. 32 new tests (333 web tests total); GPT-5.5 plan review (Critical 1+2 + Important 3 adopted) + code review (no significant issues).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| Order | ID      | Feature                                                                | Backend | Frontend | Shared | Depends on                    | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| ----- | ------- | ---------------------------------------------------------------------- | ------- | -------- | ------ | ----------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1 ✅  | I-2.2.3 | Event status indicators — upcoming, registration open/closed, sold out | ✦       | ✦        | ✦      | **I-0.1.1**                   | **Complete (2026-04-30)** — shared `EVENT_DISCOVERY_STATUSES` enum + `eventDiscoveryStatusSchema` + label map + pure `getEventDiscoveryStatus(input, now)` helper at `packages/shared/src/utils/event-discovery-status.ts`. Precedence is `event_ended > registration_closed > upcoming > sold_out > registration_open`; per-category sold-out semantics align with DEC-3 v2.2: every bounded category must be at `0`, and any unlimited category prevents `sold_out`. 19 new shared tests cover branches, parsing, precedence, capacity edges, and boundary semantics matching `registration.ts` from I-2.1.7 (`opensAt` strict `<`, `closesAt`/`endAt` `>=`). |
| 2 ✅  | I-2.2.2 | Event cards — name, date, location, price range, categories            | —       | 2 ✅     | —      | I-2.2.3                       | **Complete (2026-04-30)** — promoted SSR-safe `useNow` to `#/lib/hooks/use-now` with the event-detail import kept as a re-export; added the `events-discovery` feature module with the narrow `EventCardData` projection, compact reusable `<PublicEventCard>`, and pure date/category/location formatters. Status badge and paid "From ₹X" price are client-only behind `useNow()` / `PublicEventPriceFrom`, so cached SSR HTML omits volatile time-derived labels and prevents hydration mismatches.                                                                                                                                                          |
| 3 ✅  | I-2.2.1 | Launch-city event listing page (Coimbatore)                            | 1 ✅    | 1 ✅     | —      | I-2.2.2, I-2.2.3, **I-1.2.1** | **Complete (2026-04-30)** — added shared `eventPublicCardSchema` + `eventPublicCardCategorySchema` (replaces I-2.2.2 TS-only `EventCardData`), `GET /api/v1/events/public` route, helper extraction from `public-detail-service.ts` with no behaviour change, and `/` SSR loader + `<PublicEventsList>` grid. CDN cache contract is `s-maxage=60, stale-while-revalidate=300`. Listing is upcoming-only; past events live on organizer profile.                                                                                                                                                                                                                 |
| 4 ✅  | I-2.2.4 | Sort by date (default: upcoming first)                                 | 1 ✅    | 1 ✅     | —      | I-2.2.1                       | **Complete (2026-04-30)** — API sort enum widened to `startAtAsc \| startAtDesc` with deterministic matched-direction `startAt`/`id` ordering; `/` now stores page + sort in URL search params with `stripSearchParams` default omission, resets page to 1 on sort change, and redirects out-of-range page deep-links to the last valid page.                                                                                                                                                                                                                                                                                                                   |
...
| Order | ID      | Feature                                                                 | Backend | Frontend | Shared | Depends on                        | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ----- | ------- | ----------------------------------------------------------------------- | ------- | -------- | ------ | --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1 ✅  | I-2.3.5 | Organizer slug generation — unique, URL-safe                            | ✦       | —        | ✦      | **I-0.1.1**, **I-1.1.1**          | **Complete (2026-04-30)** — reverse-traced closeout. Functional implementation shipped in Phase 2 Readiness Sprint W1.1 (2026-04-29, migration 0015): `organizers.slug` NOT NULL + unique index, deterministic `unaccent` backfill, full `RESERVED_ORGANIZER_SLUGS` list, `OrganizerSlugService` (`reserveUniqueOrganizerSlug` / `recordOrganizerSlugRedirect` / `renameOrganizerSlug` / `isReservedOrganizerSlug`), slug generation on register and businessName-rename → `slug_redirects` write in one transaction. Closeout added 16 focused unit tests in `apps/api/test/modules/organizer/slug-service.test.ts` and `docs/impl-plan/feature-2.3-I-2.3.5.md` trace plan.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| 2 ✅  | I-2.3.1 | Organizer profile page — business name, description, verification badge | ✦       | ✦        | ✦      | I-2.3.5, **I-1.1.1**, **I-1.1.6** | **Complete (2026-04-30)** — SSR with `ssr: true`, `/organizers/:slug`. Canonical `organizerPublicProfileSchema` in `packages/shared/src/schemas/organizer-public-profile.ts` (slug, businessName, isVerified, city, description nullable max 2000) — `eventPublicOrganizerSummarySchema` re-exports it. Anonymous `GET /api/v1/organizers/by-slug/:slug` returns discriminated union `{kind:"organizer"\|"redirect"}`; redirect path verifies target organizer exists + slug-match (mirrors events) and loop-guards `newSlug === slug`. Frontend feature folder mirrors event-detail (loader → 404 / 301 / happy with `s-maxage=60, swr=300`); `noindex,nofollow` only when `!isVerified`. `<PublicOrganizerProfile>` uses `VerifiedBadge variant="inline"` + `whitespace-pre-line` description (no `dangerouslySetInnerHTML`). Description normalization (whitespace → null, surrogate-safe truncation to 2000) via shared `apps/api/src/lib/text-truncate.ts`. `PublicEventOrganizerCard` swapped to typed `<Link to="/organizers/$slug">`. 14 new API tests + 37 new web tests. See `docs/impl-plan/feature-2.3-I-2.3.1.md`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| 3 ✅  | I-2.3.4 | Verification status explanation copy                                    | —       | ✦        | —      | I-2.3.1                           | **Complete (2026-04-30)** — single-source-of-truth `VERIFICATION_EXPLANATION` in `packages/ui/src/lib/verification-copy.ts` (heading + body verbatim from `docs/requirements.md` §4.1: "verification is an onboarding and policy check, not a guarantee of event quality, safety, or specific outcomes"). New `VerificationExplainer` sibling of `verified-badge.tsx` exposes two variants: `inline-note` (shield icon + heading + body inside a subtle muted card; supports an `id` prop for anchor-linking, used as `id="about-verification"` on the public organizer profile) and `popover` (underlined "What does verified mean?" trigger that opens a Radix Popover with the same copy; used inline next to the verified badge on the event-detail organizer card so the explainer stays opt-in where vertical space is tight). Both wirings only render when `isVerified === true`. Dedicated explainer test file (7 tests) plus 2 added cases in `PublicOrganizerProfile.test.tsx` and 3 added cases in `public-event-page.test.tsx` — 495 web tests total (+12). No new deps; popover/lucide already available. Liability/support-routing copy ("report issues to EventKart support" §4.1 line 396) intentionally NOT included — that belongs to a future support-routing feature.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| 4 ✅  | I-2.3.2 | Upcoming events listing on organizer profile                            | ✦       | ✦        | —      | I-2.3.1, **I-1.2.1**              | **Complete (2026-04-30)** — extends `GET /api/v1/events/public` with optional `organizerSlug` query param (`organizerSlugSchema.optional()`), reusing the entire `listPublicEvents` pipeline (count + batched joins + projection + pagination + sort) — single source of truth for public event card listing. Filter applied via `events.organizerId IN (SELECT id FROM organizers WHERE slug = $1)` subquery on BOTH the count and rows queries (preserves correct pagination meta); unknown slug → empty `data: []`, `meta.total: 0`. Frontend adds `apps/web/src/features/organizer-detail/upcoming-events-{api.server,api,queries}.ts` and `<UpcomingEventsSection>` (1/2-col responsive grid, dashed empty state with organizer name, anchor `id="upcoming-events"`) reusing the existing `<PublicEventCard>` from events-discovery. The organizer-detail loader return type changed from `OrganizerPublicProfile` to `{ profile, events }`; events fetched via `ensureQueryData` only AFTER the profile resolves (redirect/404 still throw first), and the events fetch is **resilience-wrapped** — failure logs `console.warn` and returns `events: []` so a transient backend hiccup never 500s the profile page. Route split into `OrganizerDetailView` (pure, exported) for unit testability. Tests: 9 new API listing tests + 2 new API route tests + 5 new section tests + 5 new loader tests + route-level rendering updates. 11 new API tests + 12 new web tests. See `docs/impl-plan/feature-2.3-I-2.3.2.md`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| 5 ✅  | I-2.3.3 | Past event history on organizer profile                                 | ✦       | ✦        | —      | I-2.3.1, **I-1.2.1**              | **Complete (2026-04-30)** — extends `GET /api/v1/events/public` with optional `timeWindow: z.enum(["upcoming","past"]).default("upcoming")` (composes with I-2.3.2's `organizerSlug` filter). Past mode flips both predicates: `events.endAt <= now` AND `events.status IN ('published','completed')` — the status union mirrors `isPubliclyVisibleStatus` (public-detail-service:119, service.ts:222) so admin/cron-completed events still surface in past history while `cancelled`/`draft`/`under_review` stay private. Filter applied to BOTH the count and rows queries to keep pagination meta correct. Single-line type addition: re-exported `SQL` from `@repo/db` so the conditions array gets a static type. Frontend mirrors I-2.3.2 with `apps/web/src/features/organizer-detail/past-events-{api.server,api,queries}.ts` and `<PastEventsSection>` (heading "Past events", anchor `id="past-events"`, 1/2-col grid, dashed empty state "{name} hasn't run any past events yet.", reuses `<PublicEventCard>`, default sort `startAtDesc` so the most recent past event leads). Loader return type extended to `{ profile, upcomingEvents, pastEvents }` (renamed `events`→`upcomingEvents`); both event fetches run **concurrently** via `Promise.all` AFTER the profile resolves (redirect/404 still throw first), each wrapped in its own resilience helper so a transient failure in one list doesn't kill the page or the other list. `OrganizerDetailView` stacks Upcoming above Past inside the existing `max-w-3xl` container with `space-y-12`. Tests: 5 new API listing tests (default unchanged, past flips predicate, status union, organizerSlug+past compose, both queries get the past filter), 2 new API route tests (`timeWindow=tomorrow` → 400, valid forwards to service), 5 new past-section tests, 2 new loader tests for parallel + isolation behavior, 2 new route-level render tests. 7 new API tests + 9 new web tests. See `docs/impl-plan/feature-2.3-I-2.3.3.md`. |
| 6 ✅  | I-2.3.6 | Next-event lookup API — same-organizer only                             | ✦       | —        | ✦      | I-2.3.1, **I-1.2.1**, **I-1.2.6** | **Complete (2026-04-30)** — internal-only `GET /api/v1/organizers/{organizerId}/next-event` returning `{ success, data: EventPublicCard \| null }` for the I-6.2.3 post-event email worker. Reuses the I-2.3.2/I-2.3.3 `listPublicEvents` pipeline by adding an optional `organizerId?: string` filter to `PublicEventListingParams` (direct `eq(events.organizerId, $id)` since we have the UUID — parallel to the I-2.3.2 `organizerSlug` subquery; both filters applied to count + rows queries). Thin `selectOrganizerNextEvent` wrapper pins `limit=1, sort=startAtAsc, timeWindow=upcoming` and returns `data[0] ?? null`. New `requireInternal` preHandler (`apps/api/src/middleware/require-internal.ts`) is the first **enforcement** consumer of `request.isInternalRequest` — throws `UnauthorizedError("Internal API key required")` if the flag isn't set; the existing internal-key plugin keeps validating `x-internal-key` against `config.INTERNAL_API_KEY` via `timingSafeEqual` and remains opt-in for everything else. Existence probe runs FIRST (`organizerExistsById`) so a typo'd UUID returns 404 (operational bug) while a valid organizer with no upcoming events returns 200 + `data: null` (legitimate empty state) — the two outcomes the email worker must distinguish. Schemas (`organizerIdParamsSchema`, `organizerNextEventResponseSchema`) reuse canonical shared `eventPublicCardSchema` so the worker gets the same projection the public listing returns. **V1 scope: same-organizer only** — cross-organizer discovery deferred to product-plan §8 Layer 5 post-V1. 14 new API tests (10 route+service in `next-event.test.ts` + 4 listing-service filter tests). Validation: api check-types ✓, api 852/852 (+14 from 838) ✓, api lint ✓ (9 pre-existing warnings), shared check-types ✓; no web changes. See `docs/impl-plan/feature-2.3-I-2.3.6.md`.                                                                                                           |
...
| Order | ID      | Feature                                                                                                                                       | Backend | Frontend | Shared | Depends on                 | Notes                                                                                                                                                                                                                         |
| ----- | ------- | --------------------------------------------------------------------------------------------------------------------------------------------- | ------- | -------- | ------ | -------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1     | I-2.4.1 | ✅ Cloudflare CDN setup — DNS, SSL, caching rules for SSR pages (2026-04-30)                                                                  | —       | ✦        | —      | **I-2.1.1** or **I-2.2.1** | Ops runbook at `docs/operations/cloudflare-cdn-setup.md` + `CLOUDFLARE_*`/`CDN_BASE_URL` env scaffolding (consumed by I-2.4.2). Page Rules pinned to existing cache contract `s-maxage=60, swr=300`.                          |
| 2     | I-2.4.2 | ✅ CDN cache invalidation — Cloudflare API purge on event publish/unpublish, pricing changes, capacity changes, admin moderation (2026-04-30) | ✦       | —        | —      | I-2.4.1, **I-1.2.6**       | Fail-soft BullMQ `cdn-purge` queue (concurrency 5, 30-URL chunking, never logs API token, throws → BullMQ retries 3× exp 5s/10s/20s). Triggered AFTER tx commit from publish/unpublish/admin-approve/policy paths.            |
| 3     | I-2.4.3 | ✅ Cache stampede prevention — single-flight/Redis locking for popular event pages (2026-04-30)                                               | ✦       | —        | —      | I-2.4.1, **I-0.1.5**       | `singleFlight()` Redis SETNX-EX lock with poll-and-wait fallback; discriminated `{hit,value}\|{hit:false}` result so `null` cached values aren't misread as miss. Wired into `evt:slug:*` + `org:slug:*` (TTL 60s).           |
| 4     | I-2.4.4 | ✅ `sitemap.xml` generation + serving (2026-04-30)                                                                                            | ✦       | ✦        | —      | I-2.4.1, **I-1.2.6**       | `GET /api/v1/sitemap.xml` from `cache:sitemap:current`; future events only (`endAt > now()` AND `status='published'`); 50K-URL cap (V2 deferred to sitemap-index). `sitemap-regen` debounced queue (`removeOnComplete:true`). |
| 5     | I-2.4.5 | ✅ `robots.txt` + crawl directives (2026-04-30)                                                                                               | —       | ✦        | —      | I-2.4.1                    | Full Allow/Disallow + `Sitemap:` directive + `$` end-anchors on bare auth routes (RFC 9309 prefix-match correctness — `Disallow: /org/` doesn't block bare `/org`).                                                           |
| 6     | I-2.4.6 | ✅ 301 redirect handler for slug changes (2026-04-30 — audit-only)                                                                            | ✦       | ✦        | —      | I-2.4.1, **I-1.2.10**      | Verified invariants on HEAD: loop-guard, existence + current-slug equality (chained-rename safety), public-visibility gate, `Cache-Control: max-age=300`, 301 permanent. Already shipped with I-2.1.1 / I-2.3.1.              |
| 7     | I-2.4.7 | ✅ Canonical URL + alternate language tags (2026-04-30)                                                                                       | —       | ✦        | —      | I-2.4.1                    | `<link rel="canonical">` shipped with I-2.1.5/I-2.3.1. Adds V2-readiness `<link rel="alternate" hrefLang="en"\|"x-default">` scaffold (camelCase JSX prop avoids React 19 dev warning).                                       |
| 8     | I-2.4.8 | ✅ Breadcrumb JSON-LD on event detail (2026-04-30)                                                                                            | —       | ✦        | —      | I-2.4.1, **I-2.1.6**       | Schema.org BreadcrumbList markup: Home → Events → {event title}. Second `application/ld+json` script reusing shared `serializeJsonLdForInlineScript<T>` HTML-escape boundary.                                                 |
