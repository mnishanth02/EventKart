...
- **W1.1 — Organizer slug** (`organizers.slug`): deterministic slug generation from business name, collision-safe backfill for existing records, reserved-slug list, `OrganizerSlugService` class, `slug_redirects` writes on slug change. New files: `apps/api/src/modules/organizers/organizer-slug.service.ts`, `packages/shared/src/constants/organizer-slug.constants.ts`. Migration: `0015_organizer_slug.sql`.
...
| 8 ✅  | I-1.2.8  | Event edit & update capabilities (tiered post-publish)                                   | ✦       | ✦        | ✦      | I-1.2.1 through I-1.2.5, I-1.2.9                  | **✅ Fully complete (W1.3, 2026-04-29).** v2.2 tiered post-publish edits implemented: `publishedEventPatchSchema` for low-risk fields; `EVENT_UPDATE_PUBLISHED` audit constant with constrained metadata; structured 409 with `requiresUnpublish: true` for high-risk fields (`start_at`, `end_at`, `city`, `venue_name`, `currency`, `event_pricing_tiers.*`, `event_categories.spots_total`, `registrationOpensAt`, `registrationClosesAt`); audit-classified every event update function; `updateEventPolicies` unlocked for published events. Blanket draft-only behavior removed. CDN purge follow-up pending Cloudflare/I-2.4.2 integration. |
| 9 ✅  | I-1.2.6  | Event publish workflow — draft → published (with first-3-paid admin gate via I-1.2.7)    | ✦       | ✦        | ✦      | I-1.2.1 through I-1.2.5, **I-1.1.6**, **I-1.1.7** | Complete. **v2.2 note clarification:** organizer can self-publish when verification + Razorpay + completeness gates pass; first 3 paid events from a new organizer route through admin review (I-1.2.7) before status flips to `published`. There is no general `under_review` state for established organizers. **W1.5 (2026-04-29):** `events.first_published_at` column added (migration 0017) — set once in `publishEvent` + `adminApproveEvent`, never cleared on unpublish; `getPublishedPaidEventCount` rewritten to use it. Closes the unpublish→republish bypass of the first-3-paid admin gate (security fix). See [impl-plan](impl-plan/feature-1.2-I-1.2.6.md). |
...
- **W1.3 — Tiered post-publish edits**: `publishedEventPatchSchema`, `EVENT_UPDATE_PUBLISHED` audit constant (constrained metadata), 409 `requiresUnpublish: true` for high-risk fields, all event update functions audit-classified.
- **W1.5 — first_published_at** (`events.first_published_at`): migration 0017. Set once on first publish/admin-approve; never cleared. Closes the unpublish→republish bypass of the first-3-paid paid-event count gate (security/policy fix).
...
- `organizers` — id, user_id, business_name, **slug** (added migration 0015), description, city, email, email_verified_at, **razorpay_account_id**, verification_status, events_published_count, created_at
