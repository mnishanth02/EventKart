...
- **Set-Cookie + public-cache leak risk (auth.ts global onRequest hook) — Pending** — The global `onRequest` cookie-clear hook in `apps/api/src/plugins/auth.ts` (lines 13–83) writes `Set-Cookie` on every response. When that response also carries `Cache-Control: public` (now the case for all anonymous SSR pages via the I-2.1.1 / I-2.3.1 cache contract), shared CDNs that respect `Set-Cookie` (RFC 7234 §3) may cache the cookie and serve another user's session. Caught by adversarial review during I-2.4.6. Belongs in a dedicated infra slice — not addressed in Module 2.4. Fix candidates: gate the cookie-clear hook to authenticated paths only; or strip `Set-Cookie` on public cacheable responses; or split origins so the public CDN never sees session cookies.
