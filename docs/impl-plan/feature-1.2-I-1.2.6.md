...
```json
{
  ...
  "data": {
    ...
    "items": [
      { "check": "organizer_verified", "passed": true, "message": "Organizer verified", "severity": "error" }
    ]
  }
}
```
...
```json
{
  "success": true,
  "data": {
    "event": { /* full event object with status=draft */ },
...
export async function adminApproveEvent(deps, eventId, adminUserId): Promise<Event>
export async function adminRejectEvent(deps, eventId, adminUserId, reason?): Promise<Event>
```
