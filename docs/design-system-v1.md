...
**EventKart** (Hindi/Sanskrit for "ray of light") is a vertical SaaS platform for fitness event organizers in India. V1 targets single-day paid running events in Coimbatore, with unified registration, payments, and event-day operations. This document is the single source of truth for visual and interaction design across every EventKart surface — public discovery, registration and payment flows, the organizer dashboard, the admin panel, and the QR check-in experience.
...
```
┌──────────────────────────────────────────────────────────────┐
│  🏃 EventKart   [Coimbatore Marathon 2025 ▾]   ⌘K   🔔   👤   │  ← top bar (event-switcher prominent)
├──────────────────────────────────────────────────────────────┤
...
| Urgency       | Plain text — _"Selling fast — 12 spots left"_ — no red flame, no flashing           |
...
| OTP / payment in flight | Inline progress text — _"Sending OTP…" → "Verifying…" → "Confirmed"_                  |
...
| Surface                 | Copy                                            |
| ----------------------- | ----------------------------------------------- |
| No events match filters | _"No events match your filters."_ + reset chips |
| No participants yet     | _"Quiet at the start line."_ + share link CTA   |
| No payouts              | _"Your first payout will appear here."_         |
| Inbox empty             | _"All clear. Go for a run."_                    |
...
```css
/* ------------------------------------------------------------------
   EventKart Design System — app.css
...
@layer base {
  * {
  ...
  /* Selection */
  ::selection {
    background-color: oklch(0.72 0.185 55 / 0.35);
    color: var(--foreground);
  }
...
@layer utilities {
  ...
  .lane-line {
    box-shadow: inset 4px 0 0 0 var(--lane-color, var(--accent));
  }
...
| Accent color (Saffron)                        | **A**         | Best name-brand fit ("eventkart" = ray)              |
...
**End of EventKart Design System v1.0.0** — Ready for implementation. Updates require Design Council review.
